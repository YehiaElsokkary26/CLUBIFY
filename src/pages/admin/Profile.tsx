  ...
  return (
    <div className="phone-scroll h-[844px] pb-24" style={{ background: isDark ? '#1C1C1E' : '#FAF8F5' }}>
      ...
      <div className={cn('pt-12 pb-4 px-5 flex items-center justify-between', isDark ? 'bg-[#1C1C1E]' : 'bg-[#FAF8F5]')}>
        ...
        <h1 className={cn('text-xl font-black', isDark ? 'text-white' : 'text-[#1C1C1E]')}>My Profile</h1>
        <div className="w-9" />
      </div>
      ...
      <div className="px-5 space-y-4">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          className={cn('rounded-2xl p-5 flex flex-col items-center', isDark ? 'bg-[#2C2C2E]' : 'bg-white shadow-sm')}>
          <button onClick={() => setShowPhotoMenu(true)} className="relative group">
            ...
            <div className="absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-[#8B1A1A] flex items-center justify-center shadow-md border-2 border-white">
              <Camera size={14} className="text-white" />
            </div>
          </button>
          <h2 className={cn('text-lg font-black mt-3', isDark ? 'text-white' : 'text-[#1C1C1E]')}>{user.name}</h2>
          <p className="text-xs font-semibold text-[#8B1A1A]">{extra.position} · {adminClub.name}</p>
          <p className={cn('text-[10px] mt-0.5 font-mono', isDark ? 'text-gray-500' : 'text-gray-400')}>{user.gucId}</p>
        </motion.div>

        {/* Role at club */}
        <div className={cn('rounded-2xl p-4 space-y-3', isDark ? 'bg-[#2C2C2E]' : 'bg-white shadow-sm')}>
          <h3 className={cn('text-sm font-black', isDark ? 'text-white' : 'text-[#1C1C1E]')}>Role at club</h3>
          <Field
        ...
        {/* Personal info */}
        <div className={cn('rounded-2xl p-4 space-y-3', isDark ? 'bg-[#2C2C2E]' : 'bg-white shadow-sm')}>
          <h3 className={cn('text-sm font-black', isDark ? 'text-white' : 'text-[#1C1C1E]')}>Personal info</h3>
        ...
        {/* Contact */}
        <div className={cn('rounded-2xl p-4 space-y-3', isDark ? 'bg-[#2C2C2E]' : 'bg-white shadow-sm')}>
          <h3 className={cn('text-sm font-black', isDark ? 'text-white' : 'text-[#1C1C1E]')}>Contact</h3>
          <div>
            <label className={cn('text-xs font-bold uppercase tracking-wider mb-1.5 block flex items-center gap-1.5', isDark ? 'text-gray-400' : 'text-gray-500')}>
              <Mail size={11} /> Email
            </label>
            <div className={cn('w-full px-4 py-3 rounded-xl text-sm border-2', isDark ? 'bg-[#3A3A3C] border-[#4A4A4C] text-gray-400' : 'bg-gray-100 border-gray-200 text-gray-500')}>
              {user.email}
            </div>
          </div>
          <Field icon={<Phone size={11} />} label="Phone" value={extra.phone} onChange={(v) => setExtra((e) => ({ ...e, phone: v }))} placeholder="+20 …" />
          <Field icon={<span className="text-[10px] font-black">IG</span>} label="Instagram" value={extra.instagramHandle} onChange={(v) => setExtra((e) => ({ ...e, instagramHandle: v }))} placeholder="@yourhandle" />
          <Field icon={<span className="text-[10px] font-black">in</span>} label="LinkedIn URL" value={extra.linkedinUrl} onChange={(v) => setExtra((e) => ({ ...e, linkedinUrl: v }))} placeholder="https://linkedin.com/in/…" />
        </div>

        {/* Fun */}
        <div className={cn('rounded-2xl p-4', isDark ? 'bg-[#2C2C2E]' : 'bg-white shadow-sm')}>
          <h3 className={cn('text-sm font-black mb-3', isDark ? 'text-white' : 'text-[#1C1C1E]')}>Fun fact</h3>
          <textarea
            value={extra.funFact}
            onChange={(e) => setExtra((x) => ({ ...x, funFact: e.target.value }))}
            rows={2}
            placeholder="Something students should know about you 🎬"
            className={cn('w-full px-4 py-3 rounded-xl text-sm outline-none border-2 resize-none',
              isDark ? 'bg-[#3A3A3C] border-[#4A4A4C] text-white placeholder:text-gray-500 focus:border-[#8B1A1A]'
                     : 'bg-gray-50 border-gray-200 text-[#1C1C1E] placeholder:text-gray-400 focus:border-[#8B1A1A]')}
          />
        </div>
        ...
        <button
          ...
          className={cn('w-full py-4 rounded-2xl border-2 font-bold text-sm flex items-center justify-center gap-2',
        ...
        {showPhotoMenu && (
          <>
            ...
            >
              ...
              <button onClick={() => cameraRef.current?.click()} className={cn('w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl mb-2', isDark ? 'bg-[#3A3A3C]' : 'bg-gray-50')}>
              ...
              <button onClick={() => fileRef.current?.click()} className={cn('w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl mb-2', isDark ? 'bg-[#3A3A3C]' : 'bg-gray-50')}>
                <div className="w-9 h-9 rounded-xl bg-[#8B1A1A]/10 flex items-center justify-center">
              ...
              <button onClick={removePhoto} className={cn('w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl', isDark ? 'bg-[#3A3A3C]' : 'bg-gray-50')}>
        ...
        {showSignOutConfirm && (
          <>
            ...
            >
              ...
              <h3 className={cn('text-base font-black text-center', isDark ? 'text-white' : 'text-[#1C1C1E]')}>Sign out?</h3>
              <p className={cn('text-xs text-center mt-1', isDark ? 'text-gray-400' : 'text-gray-500')}>You'll be returned to the login screen.</p>
              <div className="flex gap-2 mt-5">
                <button onClick={() => setShowSignOutConfirm(false)}
                  className={cn('flex-1 py-3 rounded-2xl text-sm font-bold', isDark ? 'bg-[#3A3A3C] text-gray-300' : 'bg-gray-100 text-gray-700')}>
                  Cancel
                </button>
                <button onClick={handleSignOut} className="flex-1 py-3 rounded-2xl bg-red-500 text-white text-sm font-bold">
                  Sign out
                </button>
              </div>
            </motion.div>
